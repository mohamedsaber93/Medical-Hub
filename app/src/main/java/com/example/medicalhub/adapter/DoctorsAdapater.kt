...
class DoctorsAdapater (
    private var doctors: List<GetAllDoctors>,
    private val clickListener: (GetAllDoctors) -> Unit
) : RecyclerView.Adapter<DoctorsAdapater.DoctorViewHolder>() {

    var originalDoctors: List<GetAllDoctors> = doctors // Store the original list
    ...
    fun filter(query: String) {
        val filteredList = if (query.isEmpty()) {
            ...
            originalDoctors.filter { it.userName?.contains(query, ignoreCase = true) == true }
        }
